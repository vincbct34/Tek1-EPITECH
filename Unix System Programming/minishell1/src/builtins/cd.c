...
static void get_home(minishell *msh)
{
...
{
    if (access(msh->args[1], F_OK) == -1) {
        my_puterror(msh->args[1]);
        my_puterror(": No such file or directory.\n");
        msh->return_value = 1;
        return;
    }
    if (access(msh->args[1], X_OK) == -1) {
        my_puterror(msh->args[1]);
        my_puterror(": Permission denied.\n");
        msh->return_value = 1;
        return;
    }
    if (access(msh->args[1], F_OK) != -1 && access(msh->args[1], R_OK) != -1) {
        my_puterror(msh->args[1]);
...
void my_cd(minishell *msh)
{
    ...
    if (msh->args[1] != NULL && my_strcmp(msh->args[1], "-") == 0) {
        chdir(msh->oldpwd);
        msh->oldpwd = msh->pwd;
        msh->pwd = getcwd(NULL, 0);
        msh->oldpwd = msh->pwd;
