...
static char *get_path(char **array, char **env)
{
    char *path = NULL;
    char *full_path = NULL;
    char *path_copy = NULL;
...
static void error_gest_segfault(int status, minishell *msh)
{
    if (WIFEXITED(status)) {
...
static void is_a_directory(minishell *msh)
{
    ...
    if (stat(msh->args[0], &sb) == 0 && S_ISDIR(sb.st_mode)) {
        my_puterror(msh->args[0]);
...
void my_exec(minishell *msh)
{
    ...
    if (pid == 0) {
        is_a_directory(msh);
        path = get_path(msh->args, msh->env);
        if (execve(path, msh->args, msh->env) && errno == ENOEXEC) {
            my_puterror(path);
